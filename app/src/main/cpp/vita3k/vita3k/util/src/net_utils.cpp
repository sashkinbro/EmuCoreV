...
#ifdef __ANDROID__
    LOG_WARN("get_web_response is not available in Android native build: {}", url);
    return {};
#else
...
#ifdef __ANDROID__
    LOG_WARN("download_file is not available in Android native build: {} -> {}", url, output_file_path);
