...
static bool can_use_stock_gui_for_install(GuiState *gui) {
...
static void set_theme_name(EmuEnvState &emuenv, vfs::FileBuffer &buf) {
    emuenv.app_info.app_title = get_theme_title_from_buffer(buf);
...
static bool install_archive_content(EmuEnvState &emuenv, GuiState *gui, const ZipPtr &zip, const std::string &content_path, const std::function<void(ArchiveContents)> &progress_callback) {
    ...
    const auto use_gui = can_use_stock_gui_for_install(gui);
    if (!created) {
        if (!use_gui || gui->file_menu.archive_install_dialog) {
            fs::remove_all(output_path);
    ...
    if (use_gui && !gui->file_menu.archive_install_dialog && (emuenv.app_info.app_category != "theme")) {
...
static std::vector<std::string> get_archive_contents_path(
    const ZipPtr &zip,
    const std::function<void(ArchiveContents)> &progress_callback = nullptr) {
    mz_uint num_files = mz_zip_reader_get_num_files(zip.get());
...
std::vector<ContentInfo> install_archive(EmuEnvState &emuenv, GuiState *gui, const fs::path &archive_path, const std::function<void(ArchiveContents)> &progress_callback) {
    ...
    const auto content_path = get_archive_contents_path(zip, progress_callback);
...
static bool install_content(EmuEnvState &emuenv, GuiState *gui, const fs::path &content_path) {
    ...
    const auto use_gui = can_use_stock_gui_for_install(gui);
    if (use_gui && ((emuenv.app_info.app_category.find("gd") != std::string::npos) || (emuenv.app_info.app_category.find("gp") != std::string::npos))) {
        gui::init_user_app(*gui, emuenv, emuenv.app_info.app_title_id);
        gui::save_apps_cache(*gui, emuenv);
    }

    if (use_gui && (emuenv.app_info.app_category != "theme"))
        gui::update_notice_info(*gui, emuenv, "content");
...
uint32_t install_contents(EmuEnvState &emuenv, GuiState *gui, const fs::path &path) {
    ...
    if (installed && can_use_stock_gui_for_install(gui)) {
        gui::save_apps_cache(*gui, emuenv);
...
#if defined(__ANDROID__) && defined(EMUCOREV_ANDROID_FRONTEND)
    const bool allow_switch_state = false;
#else
