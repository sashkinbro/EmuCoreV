...
#if defined(__ANDROID__) && defined(EMUCOREV_ANDROID_FRONTEND)
static constexpr bool kUseStockAndroidGui = false;
#elif defined(__ANDROID__)
static constexpr bool kUseStockAndroidGui = true;
#endif

static bool use_stock_shell_features() {
#if defined(__ANDROID__) && defined(EMUCOREV_ANDROID_FRONTEND)
    return false;
#else
    return true;
...
static bool use_stock_shell_ui() {
#if defined(__ANDROID__) && defined(EMUCOREV_ANDROID_FRONTEND)
    return false;
#else
    return true;
...
int main(int argc, char *argv[]) {
    ...
    Config cfg{};
    EmuEnvState emuenv;
    app::attach_runtime_emuenv(&emuenv);
    const auto config_err = config::init_config(cfg, argc, argv, root_paths);

    // Guard against an empty pref path. boost::filesystem::create_directories
    // throws EINVAL on an empty path and uncaught exceptions terminate the
    // process, which produced the first-launch crash on Android when
    // getExternalFilesDir briefly returned null before the app's data dir
    // was warmed up.
    if (cfg.get_pref_path().empty()) {
        if (!root_paths.get_pref_path().empty()) {
            cfg.set_pref_path(root_paths.get_pref_path());
        } else {
            LOG_CRITICAL("Preference path is empty; cannot create required directories. Aborting boot cleanly.");
            return InitConfigFailed;
        }
    }
...
#else
        // EmuCoreV owns setup on Android, so the stock Vita3K wizard is disabled in frontend mode.
        if (!emuenv.cfg.initial_setup)
            LOG_INFO("Skipping stock Vita3K initial setup on Android.");
#endif
        gui::init(gui, emuenv);
    ...
    if (cfg.content_path.has_value()) {
        ...
        const auto is_rif = (extention == ".rif") || (cfg.content_path->filename() == "work.bin");
...
#if defined(__ANDROID__) && defined(EMUCOREV_ANDROID_FRONTEND)
    // EmuCoreV owns the Android shell, so the native core should only boot a concrete title.
    if (!cfg.console && run_type == app::AppRunType::Unknown) {
        LOG_ERROR("Android launch requested without a title id. Stock Vita3K GUI is disabled in EmuCoreV.");
        return QuitRequested;
    }
#endif
