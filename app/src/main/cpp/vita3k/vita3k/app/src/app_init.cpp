...
namespace app {
namespace {
constexpr float kFontScaleCandidates[] = { 1.f, 1.5f, 2.f };

#ifdef __ANDROID__
fs::path resolve_android_storage_path() {
    ...
    if (env && activity) {
        ...
        if (clazz) {
            jmethodID method_id = env->GetMethodID(clazz, "getBaseStoragePath", "()Ljava/lang/String;");
            if (method_id) {
                auto j_storage_path = static_cast<jstring>(env->CallObjectMethod(activity, method_id));
                if (j_storage_path) {
                    const char *storage_chars = env->GetStringUTFChars(j_storage_path, nullptr);
                    if (storage_chars && storage_chars[0] != '\0') {
                        fs::path resolved_path = fs_utils::utf8_to_path(storage_chars);
                        env->ReleaseStringUTFChars(j_storage_path, storage_chars);
    ...
    const char *fallback_storage = SDL_GetAndroidExternalStoragePath();
    if (fallback_storage && fallback_storage[0] != '\0') {
        return fs_utils::utf8_to_path(fallback_storage) / "";
    }
...
void update_viewport(EmuEnvState &state) {
    ...
    for (int i = 0; i <= state.max_font_level; i++) {
        if (i == state.max_font_level || scale <= kFontScaleCandidates[i]) {
            state.current_font_level = i;
            break;
        }
        if (kFontScaleCandidates[i] / scale > scale / kFontScaleCandidates[i + 1]) {
            state.current_font_level = i;
            break;
        }
    }
...
#ifdef __ANDROID__
    fs::path storage_path = resolve_android_storage_path();
    if (storage_path.empty()) {
        storage_path = fs::temp_directory_path() / "emucorev" / "";
    }
...
void switch_state(EmuEnvState &emuenv, const bool pause) {
    if (pause) {
        ...
        app::set_controller_overlay_state(0);
#endif
        emuenv.kernel.pause_threads();
    } else {
#ifdef __ANDROID__
        emuenv.display.imgui_render = false;
        app::set_controller_overlay_state(app::get_overlay_display_mask(emuenv.cfg));
#endif
