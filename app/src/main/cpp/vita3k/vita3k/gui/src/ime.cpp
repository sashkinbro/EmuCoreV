...
static void set_second_keyboard(Ime &ime) {
    ime.lang.keyboard_pos = { { FIRST, 13.f }, { SECOND, 60.f }, { THIRD, 154.f } };
    ime.lang.key = {
...
void draw_ime(Ime &ime, EmuEnvState &emuenv) {
    ...
    const auto KEY_BUTTON_SIZE = ImVec2(ime.lang.size_key * SCALE.x, BUTTON_HEIGHT_SIZE);
    const auto ENTER_BUTTON_SIZE = ImVec2(135.f * SCALE.x, BUTTON_HEIGHT_SIZE);
    const auto BUTTON_SIZE = numeric_pad ? ENTER_BUTTON_SIZE : ImVec2(ime.lang.size_button * SCALE.x, BUTTON_HEIGHT_SIZE);
    ...
    if (numeric_pad) {
        ...
        for (const auto &keyboard : is_shift ? ime.lang.shift_key : ime.lang.key) {
            for (uint32_t i = 0; i < keyboard.second.size(); i++) {
                if (!i)
                    ImGui::SetCursorPos(ImVec2(ime.lang.keyboard_pos[keyboard.first] * SCALE.x, key_row_pos[keyboard.first] * SCALE.y));
        ...
        ImGui::PushStyleColor(ImGuiCol_Button, GUI_COLOR_TEXT);
        ImGui::PushStyleColor(ImGuiCol_Text, GUI_COLOR_TEXT_BLACK);
        const auto &ponct_1 = is_shift ? ime.lang.punct[FIRST][YES] : ime.lang.punct[FIRST][NO];
        ...
        ImGui::PushStyleColor(ImGuiCol_Button, GUI_COLOR_TEXT);
        ImGui::PushStyleColor(ImGuiCol_Text, GUI_COLOR_TEXT_BLACK);
        const auto &ponct_2 = is_shift ? ime.lang.punct[SECOND][YES] : ime.lang.punct[SECOND][NO];
        if (ImGui::Button(string_utils::utf16_to_utf8(ponct_2).c_str(), PUNCT_BUTTON_SIZE))
            update_ponct(ime, ponct_2);
        ImGui::PopStyleColor(2);
        if (!ime.lang.second_keyboard.empty()) {
            ImGui::SameLine(0, SPACE);
            ImGui::PushStyleColor(ImGuiCol_Button, IME_BUTTON_BG);
            ImGui::PushStyleColor(ImGuiCol_Text, GUI_COLOR_TEXT);
            if (ImGui::Button(ime.lang.second_keyboard[current_keyboard].c_str(), PUNCT_BUTTON_SIZE)) {
                const auto is_first = current_keyboard == FIRST;
                if (is_first)
                    set_second_keyboard(ime);
                else
                    ::ime::init_ime_lang(ime, static_cast<SceImeLanguage>(emuenv.cfg.current_ime_lang));
    ...
    if (!numeric_pad && emuenv.cfg.ime_langs.size() > 1) {
        ...
        if (ImGui::BeginPopup("S/K", ImGuiWindowFlags_NoMove)) {
            for (const auto &lang : emuenv.cfg.ime_langs) {
                if (ImGui::MenuItem(::ime::get_ime_lang_index(ime, static_cast<SceImeLanguage>(lang))->second.c_str(), nullptr, emuenv.cfg.current_ime_lang == lang)) {
                    ::ime::init_ime_lang(ime, static_cast<SceImeLanguage>(lang));
    ...
    if (ImGui::Button(ime.lang.space_str.c_str(), SPACE_BUTTON_SIZE) || ImGui::IsKeyPressed(static_cast<ImGuiKey>(emuenv.cfg.keyboard_button_triangle)))
        update_ponct(ime, u" ");
    ImGui::PopStyleColor();
