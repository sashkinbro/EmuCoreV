...
void draw_shader_caching_warn(GuiState &gui, EmuEnvState &emuenv) {
    ImGui::SetNextWindowPos(ImVec2(emuenv.drawable_viewport_pos.x + (2.f * emuenv.manual_dpi_scale), emuenv.drawable_viewport_pos.y + (2.f * emuenv.manual_dpi_scale)));
    ImGui::SetNextWindowBgAlpha(0.6f);
    ImGui::Begin("##shaders_caching_warn", nullptr, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_AlwaysAutoResize);
    ImGui::Text("%s", gui.lang.compile_shaders["caching_shaders"].c_str());
    ImGui::End();
}
