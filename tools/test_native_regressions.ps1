...
$includes = @('vita3k/config/include', 'vita3k/renderer/include', 'vita3k/packages/include', 'vita3k/ime/include', 'vita3k/mem/include', 'vita3k/io/include', 'vita3k/util/include', 'external/boost', 'external/fmt/include', 'external/spdlog/include', 'external/pugixml/src', 'external/VulkanMemoryAllocator-Hpp/Vulkan-Headers/include')
...
$xmlSource = Join-Path $vendor 'external/pugixml/src/pugixml.cpp'
$gameDb = Join-Path $repoRoot 'app/src/main/assets/compatibility/game-db.xml'
$compileCommand = 'call "' + $vcvars + '" >nul && cl.exe /nologo /std:c++20 /EHsc /utf-8 /Zc:__cplusplus /DFMT_HEADER_ONLY /DSPDLOG_FMT_EXTERNAL /DBOOST_ALL_NO_LIB ' + $includeArgs + ' "' + $testSource + '" "' + $sfoSource + '" "' + $imeSource + '" "' + $xmlSource + '" /Fe:native_core_regressions.exe'
Push-Location $outputDir
try {
    & cmd.exe /d /s /c $compileCommand
    if ($LASTEXITCODE -ne 0) { throw "Native regression build failed: $LASTEXITCODE" }
    & (Join-Path $outputDir 'native_core_regressions.exe') $gameDb
